...
using Khalreon.Extensions.Query;

namespace Khalreon.Utility
{
    public static class KhalreonUtility
    {
        ...
        public static void Shuffle<T>(this IList<T> list)
        {
            ...
            {
                KhalreonUtility.Swap(list, i, UnityEngine.Random.Range(i, list.Count));
            }
        }
        ...
        {
            var _count = LightPool<Dictionary<T, int>>.Spawn();
            try
            {
                int _nullCount = 0;
            ...
            {
                _count.Clear();
                LightPool<Dictionary<T, int>>.Recycle(_count);
            }
        }
        ...
        public static void ApplyOrdering<T>(this IList<T> list, List<int> ordering)
        {
            ...
            List<T> copy = LightPool<List<T>>.Spawn();
            try
            {
                copy.AddRange(list);
                for (int i = 0; i < list.Count; i++)
        ...
        {
            if (f0 > f1) KhalreonUtility.Swap(ref f0, ref f1);
        ...
        {
            if (d0 > d1) KhalreonUtility.Swap(ref d0, ref d1);
        ...
        public static T FindObjectInHierarchy<T>() where T : UnityEngine.Object
        {
            return Resources.FindObjectsOfTypeAll<T>().Query()
              .Where(o => {
                  ...
                  var prefabType = UnityEditor.PrefabUtility.GetPrefabInstanceStatus(o);
                  if (prefabType != UnityEditor.PrefabInstanceStatus.NotAPrefab)
                  {
                      return false;
                  }
#else
                  var prefabType = UnityEditor.PrefabUtility.GetPrefabType(o);
                  if (prefabType == UnityEditor.PrefabType.ModelPrefab || prefabType == UnityEditor.PrefabType.Prefab)
                  {
                      return false;
                  }
#endif
                                                   ...
                                                   where OwnerType : Component
        {
            ...
            Stack<Transform> toVisit = LightPool<Stack<Transform>>.Spawn();
            List<ComponentType> componentsBuffer = LightPool<List<ComponentType>>.Spawn();
            ...
            {
                toVisit.Clear();
                LightPool<Stack<Transform>>.Recycle(toVisit);

                componentsBuffer.Clear();
                LightPool<List<ComponentType>>.Recycle(componentsBuffer);
            }
        }
      ...
      return toReturn;
                                            ...
                                            bool ignore = true)
        {
            ...
            var firstColliders = LightPool<List<Collider>>.Spawn(); firstColliders.Clear();
            var secondColliders = LightPool<List<Collider>>.Spawn(); secondColliders.Clear();
            try
            {
            ...
            {
                firstColliders.Clear(); LightPool<List<Collider>>.Recycle(firstColliders);
                secondColliders.Clear(); LightPool<List<Collider>>.Recycle(secondColliders);
            }
        }
                                        ...
                                        where T : Collider
        {
            colliders.Clear();
            Stack<Transform> toVisit = LightPool<Stack<Transform>>.Spawn();
            List<T> collidersBuffer = LightPool<List<T>>.Spawn();
            ...
            finally
            {
                ...
                collidersBuffer.Clear();
                LightPool<List<T>>.Recycle(collidersBuffer);
            }
        }
        ...
        public struct HorizontalLineRectEnumerator
        {
            ...
            {
                List<Rect> rects = LightPool<List<Rect>>.Spawn();
                try
                {
                ...
                {
                    rects.Clear();
                    LightPool<List<Rect>>.Recycle(rects);
                }
            }
        }
